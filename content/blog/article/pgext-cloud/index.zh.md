...
tags: [ext, repo, catalog, cli]
---

> [微信公众号链接](https://mp.weixin.qq.com/s/oHHzhbbt5suSxnJhyxTwQQ)

PostgreSQL 拥有极为强大的扩展插件体系。
在 《[PostgreSQL 正在吞噬数据库世界](https://vonng.com/pg/pg-eat-db-world/)》中， 老冯已经阐述过  **可扩展性** 是 PostgreSQL 成功的核心要素。
...
[![PostgreSQL 扩展生态全景图](/article/pgext-cloud/ecosystem.webp)](https://vonng.com/pg/pg-eat-db-world)

然而，要在生产环境中可靠地编译、安装这些扩展插件并非易事，会有许许多的挑战。绝大多数扩展仅仅交付源代码，需要自己去摸索编译。
对于严肃的生产环境来说，源码编译和 [Docker 镜像](https://vonng.com/db/pg-in-docker/) 往往也不是一个可行选项 ——
当你需要组合使用多个不同扩展，在不同服务器上精确安装相同版本，或者根本不想使用容器时，就会有各种问题。
还有一些非技术的挑战需要克服 —— [官方镜像站停止更新](/zh/article/pg-mirror-pigsty/) ，以及国内网络环境受限导致无法下载。
...
![老冯维护与修复 PostgreSQL 扩展的 GitHub 贡献记录](/article/pgext-cloud/contrib.webp)
...
![长期未更新的阿里云 PostgreSQL 镜像目录](/article/pgext-cloud/aliyun.webp)
...
![PGEXT.CLOUD 的全球与中国大陆软件包分发路径](/article/pgext-cloud/distribution.webp)
...
![PGEXT.CLOUD 扩展目录首页](/article/pgext-cloud/homepage.webp)

PGEXT.CLOUD 的[扩展目录](https://pgext.cloud/)汇集了前述 431 个扩展的详细信息。
我们为每个扩展整理了元数据、在各操作系统和 PG 版本上的可用性矩阵，以及完整的安装、配置与加载使用说明。

![PGEXT.CLOUD 扩展安装与使用说明](/article/pgext-cloud/usage.webp)

![PGEXT.CLOUD 扩展配置与加载说明](/article/pgext-cloud/instruct.webp)
...
![使用 PIG 构建 PostgreSQL 扩展软件包](/article/pgext-cloud/build.webp)
...
[![Pigsty 支持的 PostgreSQL 分支内核](/article/pgext-cloud/kernel.webp)](https://pigsty.cc/docs/pgsql/kernel/)
...
![PGEXT.CLOUD 解锁 PostgreSQL 扩展生态](/article/pgext-cloud/featured.webp)

---

> **归档说明（2026-08-30）：** 本文原载 [vonng.com](https://vonng.com/pg/pgext-cloud/)。文中的软件包数量、截图与上下文以原始发表日期为准；当前行为请以 [PIG 文档](/zh/docs/)和实时[扩展目录](https://pigsty.cc/ext/)为准。

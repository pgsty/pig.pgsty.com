...
![SOW 中文项目主页](/article/sow/sow-home.webp)
...
这个名字值得单独讲讲。此前，我们做过另一个配套开源项目：[**Pig —— PostgreSQL Install Genius**](/zh/article/pig/)；它是 PostgreSQL 生态的包管理器。既然有小猪负责装包，那么制作这些包、承载、组织与分发软件制品的仓库工具，自然就是“母猪” SOW 了。

![母猪造型的 USB 集线器](/article/sow/sow-usb-hub.webp)
...
![Pig iron 与母猪、小猪命名的历史渊源](/article/sow/pig-iron.webp)
...
![Pigsty 使用 SOW 替换 RPM 与 DEB 仓库工具链](/article/sow/pigsty-offline-diff.webp)

到了 [Pigsty 4.5](https://vonng.com/pigsty/v4.5/)，我把这套冗余清掉了。SOW 是一个几 MB 级的自包含二进制，在 Linux 和 macOS 上都能直接运行，同时理解 RPM / DEB 包格式与 APT / DNF 仓库规范。它没有守护进程，也不需要额外的语言运行时。
...
![传统仓库同步需要花费大量时间遍历和比较文件](/article/sow/rclone-sync.webp)
...
![SOW 的自包含二进制与增量发布能力](/article/sow/sow-features.webp)
...
![SOW Plain 平面仓库文档](/article/sow/plain-repo-docs.webp)
...
![SOW Managed 模式的四层结构](/article/sow/managed-hierarchy.webp)
...
![SOW 的包池与 RPM、APT 元数据视图](/article/sow/package-pool.webp)
...
![SOW 的 Plain 与 Managed 能力矩阵](/article/sow/sow-capabilities.webp)
...
![SOW 命令索引](/article/sow/sow-commands.webp)
...
> **归档说明（2026-08-30）：** 本文原载 [vonng.com](https://vonng.com/db/sow/)。文中的软件包数量、截图与上下文以原始发表日期为准；当前行为请以 [PIG 文档](/zh/docs/)和实时[扩展目录](https://pigsty.cc/ext/)为准。

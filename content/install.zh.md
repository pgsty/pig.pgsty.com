...
你也可以直接从[当前 GitHub Release](https://github.com/pgsty/pig/releases/tag/v{{< param version >}}) 或 Pigsty 软件仓库下载 `pig` 安装包（`RPM`/`DEB`/压缩包）。`v{{< param version >}}` 发布产物使用以下仓库直链格式：
...
pig update -v {{< param version >}}   # 升级到指定版本
```

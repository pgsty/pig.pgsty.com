...
- Use [`pig pb restore`](/pb/#pb-restore) when you need lower-level restore semantics or fine-grained scripting control.
- Use [`pig pt switchover CLUSTER`](/pt/) or [`pig pt failover CLUSTER --candidate MEMBER`](/pt/) when you need manual Patroni cluster switching (native patronictl passthrough since v1.6.0; the cluster name is required).

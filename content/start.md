...
These commands install **host packages**. They do not run `CREATE EXTENSION`, change `shared_preload_libraries`, or restart PostgreSQL. After installation, run `pig ext info pg_duckdb` and `pig ext info vector`, apply the displayed preload/DDL steps to the intended database, and validate the service before production use.

## Installation

You can [**install**](/install/) `pig` with the following command:
...
```bash
$ pig version
pig version {{< param version >}} linux/amd64
```
...
────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
timescaledb         available  2.29.1      TIME   -dslt-  Timescale       PIGSTY   16-18  postgresql-18-timescaledb-tsl         Enables scalable inserts and complex queries for time-series dat
...
This file is updated with each pig release. You can update it directly using the [`pig ext reload`](/ext/#ext-reload) command.
The updated file is placed in `~/.pig/extension.csv` by default. You can view and modify it, and the latest online catalog is available at [**repo.pigsty.io/ext/data/extension.csv**](https://repo.pigsty.io/ext/data/extension.csv).

The listing above is a catalog snapshot, not a guarantee that each entry is available for every target. Use `pig ext avail NAME` on the actual host to inspect its PostgreSQL/OS/architecture matrix.

## Add Repositories

To install extensions, you first need to add upstream repositories. [`pig repo`](/repo/) can be used to manage Linux APT/YUM/DNF software repository configuration.

You can use the straightforward [`pig repo set`](/repo/#repo-set) to overwrite existing repository configuration, ensuring only necessary repositories exist in the system:
...
Or choose the gentler [`pig repo add`](/repo/#repo-add) to add needed repositories:
...
After adding repositories, you can use [`pig ext add`](/ext/#ext-add) to install extensions (and related packages):
...
[Alias translation list for EL](https://github.com/pgsty/pig/blob/main/cli/ext/catalog.go#L231)
...
[Alias translation list for Debian/Ubuntu](https://github.com/pgsty/pig/blob/main/cli/ext/catalog.go#L312)
...
[`pig ext status`](/ext/#ext-status) shows installed extensions.
...
[`pig ext scan`](/ext/#ext-scan) provides a lower-level scan. It scans shared libraries under the target PG directory to discover installed extensions:

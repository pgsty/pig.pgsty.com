...
以上命令安装的是 **主机软件包**，不会执行 `CREATE EXTENSION`，不会修改 `shared_preload_libraries`，也不会重启 PostgreSQL。安装后请运行 `pig ext info pg_duckdb` 与 `pig ext info vector`，按照提示在目标数据库中完成预加载与 DDL，并在投入生产前验证服务。

## 安装

您可以使用以下命令 [**一键安装**](/zh/install/) `pig`：
...
这份文件会随着 pig 版本发布不断更新，您可以直接使用 [`pig ext reload`](/zh/ext/#ext-reload) 命令更新这份数据文件。
更新后的文件会默认放置于 `~/.pig/extension.csv` 中，您可以查阅与更改；在线最新版目录可在 [**repo.pigsty.cc/ext/data/extension.csv**](https://repo.pigsty.cc/ext/data/extension.csv) 获取。

上面的列表只是目录快照，并不保证每个条目在所有目标组合中都有软件包。请在实际主机上运行 `pig ext avail NAME`，查看对应 PostgreSQL、操作系统与架构的可用矩阵。

## 添加仓库

要想安装扩展，首先需要添加上游仓库。[`pig repo`](/zh/repo/) 可用于管理 Linux APT/YUM/DNF 软件仓库配置。

您可以使用简单粗暴直接的版本 [`pig repo set`](/zh/repo/#repo-set) 覆盖式写入现有仓库配置，该命令确保系统中只存在必须的仓库配置：
...
或者选择使用温和的 [`pig repo add`](/zh/repo/#repo-add) 添加所需的仓库：
...
添加仓库后，您可以使用 [`pig ext add`](/zh/ext/#ext-add) 子命令安装扩展（以及相关软件包）
...
[EL 使用的别名翻译列表](https://github.com/pgsty/pig/blob/main/cli/ext/catalog.go#L231)
...
[Debian / Ubuntu 系统使用的别名翻译](https://github.com/pgsty/pig/blob/main/cli/ext/catalog.go#L312)
...
[`pig ext status`](/zh/ext/#ext-status) 命令可以用于显示当前安装的扩展。
...
[`pig ext scan`](/zh/ext/#ext-scan) 提供更底层的扩展扫描功能，将扫描指定 PostgreSQL 目录下的共享库，从而发现安装了哪些扩展：
